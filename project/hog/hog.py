...
def boar_brawl(player_score, opponent_score):
    ...
    second_last = opponent_score //10 % 10
    score = 3 * abs(second_last - last)
    if score == 0:
        score = 1
    return score
    # END PROBLEM 2
...
def sus_points(score):
    ...
    if num_factors(score) == 3 or num_factors(score) == 4:
        next_score = score
        while is_prime (next_score) == False:
            next_score += 1
        return next_score
    else:
        return score
...
def make_averaged(original_function, samples_count=1000):
    ...
    def f(*arg):
        ...
        for i in range (samples_count):
            sum = sum + original_function(*arg)
        ave =sum / samples_count
        return ave
    return f 
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_num_rolls = 0
    for i in range(1, 11):
        f = make_averaged(roll_dice, samples_count)
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    last = score % 10
    second_last = opponent_score //10 % 10
    boar_score = 3 * abs(second_last - last)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    second_last = opponent_score // 10 % 10
    boar_score = 3 * abs(second_last - last)
    sus_boar_score = sus_points(boar_score + score)
    add_score = sus_boar_score - score
