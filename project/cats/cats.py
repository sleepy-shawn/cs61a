...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    all_choices= [i for i in paragraphs if select(i) == True]
    if len(all_choices) <= k:
        return''
    else:
        return all_choices[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(i):
        new_i = split(remove_punctuation(lower(i)))
        for t in new_i:
            if t in subject:
                return True
        return False
...
def accuracy(typed, source):
    ...
    if typed_words == []:
        if source_words == []:
            ...
            return 0.0  
    return check(typed_words,source_words) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    lowest_diff = limit
    for i in word_list:
        if diff_function(typed_word, i, limit) < lowest_diff:
            lowest_diff = diff_function(typed_word, i, limit)
            result = i
...
def feline_fixes(typed, source, limit):
    ...
    if source == '' and typed == '':
        ...
        return feline_fixes(typed[1:], source, limit -1) + 1
    elif typed =='':
        return feline_fixes(typed, source[1:], limit -1) + 1
    else:
        if source[0] == typed[0]:
...
def minimum_mewtations(typed, source, limit):
    ...
    if not typed and not source:
        ...
        add = minimum_mewtations(typed[0:], source[1:], limit - 1)
        remove = minimum_mewtations(typed[1:], source[0:], limit - 1)
