...
class Account:
    ...
    def deposit(self, amount):
        ...
        after = self.balance
        win = Transaction(id, before, after)
        self.transactions.append(win)
        self.id = self.id + 1
        return self.balance
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            lose = Transaction(id, before, before)
            self.transactions.append(lose)
            self.id = self.id + 1
            return 'Insufficient funds'
        self.balance = self.balance - amount
        after = self.balance
        lose = Transaction(id, before, after)
        self.transactions.append(lose)
        self.id = self.id + 1
        return self.balance
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    elif not make_change(amount - smallest, rest):
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        if not make_change(coin, self.coins):
            ...
            copy = make_change(coin, self.coins) # for return
            for i in copy:
